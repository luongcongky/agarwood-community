...
type Props = {
  ...
  locale: Locale
  q?: string
  /** Skip offset ban đầu trong DB — đếm từ các bài hero/sub-hero đã render ở
   *  server. Load more sẽ skip = offsetStart + items.length. */
  offsetStart: number
  /** Labels i18n-free (caller truyền xuống từ server). */
  pinnedLabel: string
  emptyLabel: string
  loadingLabel: string
  endLabel: string
...
/**
 * Client component bọc danh sách tin "Mới nhất" với infinite scroll.
 * IntersectionObserver với rootMargin 200px để load trước khi user chạm đáy.
 *
 * Dates từ server action có thể là Date (direct Prisma) hoặc string (nếu đi
 * qua cache layer) — normalize trước khi format.
 */
export function LatestNewsList({
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      // silent: user có thể reload hoặc scroll lại để trigger observer.
    } finally {
      setLoading(false)
    }
