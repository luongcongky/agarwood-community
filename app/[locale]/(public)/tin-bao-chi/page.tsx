...
const PAGE_TITLE = "Tin báo chí"
const PAGE_META_TITLE = "Tin báo chí ngoài | Hội Trầm Hương Việt Nam"
const PAGE_META_DESC =
  "Điểm tin báo chí từ các nguồn báo lớn về cây dó bầu, trầm hương, ngành trầm hương Việt Nam — biên tập viên hội tổng hợp + dẫn link bài gốc."
...
const getDefaultExternalNewsList = unstable_cache(
  ...
  ["tin-bao-chi_list_default"],
  { revalidate: 300, tags: ["news", "external"] },
)
...
const getSidebarFeaturedExternalNews = unstable_cache(
  ...
  ["tin-bao-chi_sidebar_featured"],
  { revalidate: 600, tags: ["news", "external"] },
)

async function SidebarFeaturedBlock({ title, locale }: { title: string; locale: Locale }) {
  const items = await getSidebarFeaturedExternalNews()
  return <SidebarList title={title} items={items} locale={locale} itemHrefPrefix="/tin-bao-chi" />
}
...
export async function generateMetadata() {
  return {
    ...
    alternates: hreflangAlternates("/tin-bao-chi"),
  }
}
...
export default async function ExternalNewsPage({
  ...
  const listingJsonLd = {
    ...
    description: PAGE_META_DESC,
    url: localizedUrl("/tin-bao-chi", locale),
    ...
    mainEntity: {
      ...
      itemListElement: list.slice(0, heroConsumed + LIST_PAGE_SIZE).map((item, idx) => ({
        "@type": "ListItem",
        position: idx + 1,
        url: localizedUrl(`/tin-bao-chi/${item.slug}`, locale),
    ...
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
      ...
      <div className="lg:grid lg:grid-cols-12 lg:gap-x-10">
        <div className="min-w-0 lg:col-span-9 lg:col-start-1 lg:row-start-1">
          <Section
            title={PAGE_TITLE}
            titleHref="/tin-bao-chi"
            rightNav={
              <form method="GET" action="/tin-bao-chi" className="flex items-center gap-2">
                <input
                  type="text"
                  name="q"
                  defaultValue={q}
                  placeholder="Tìm kiếm tin báo chí..."
                ...
                {isSearch && (
                  <Link href="/tin-bao-chi" className="text-[11px] uppercase tracking-wide text-neutral-500 hover:text-brand-700 hover:underline">
                    ✕
                  </Link>
                )}
              </form>
          ...
          >
            ...
            {heroItem && (
              <div className="mb-10">
                <Link href={`/tin-bao-chi/${heroItem.slug}`} className="group block lg:grid lg:grid-cols-3 lg:gap-0">
                ...
                {subHeroItems.length > 0 && (
                  <div className="mt-8 grid gap-6 border-t border-neutral-200 pt-8 sm:grid-cols-3">
                    {subHeroItems.map((item) => (
                      <Link key={item.id} href={`/tin-bao-chi/${item.slug}`} className="group block">
        ...
        <div className="mt-10 min-w-0 lg:col-span-9 lg:col-start-1 lg:row-start-2 lg:mt-10">
          ...
          <LatestExternalNewsList
            ...
            endLabel="Đã hiển thị tất cả tin báo chí"
          />
        </div>
      </div>
    </div>
