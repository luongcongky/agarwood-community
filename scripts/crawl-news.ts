/**
 * Crawl tin tức Hội từ website cũ hoitramhuongvietnam.org
 *
 * Workflow per article:
 *   1. Đọc metadata từ bang-tin-hoi-articles.json (title, date, thumbnail, link)
 *   2. Fetch HTML bài viết từ link gốc
 *   3. Parse JSDOM → extract main content (.single-post / .blog_details / article)
 *   4. Upload thumbnail → Cloudinary (news/thumbnails/)
 *   5. Upload inline images → Cloudinary (news/{slug}/)
 *   6. Upload file đính kèm (PDF/DOC) → Google Drive (folder "Bang tin Hoi")
 *   7. Clean HTML + DOMPurify sanitize
 *   8. Insert vào DB (news table) via Prisma
 *
 * Usage:
 *   npx tsx scripts/crawl-news.ts              # crawl tất cả
 *   npx tsx scripts/crawl-news.ts --force      # crawl lại (xóa cũ)
 *   npx tsx scripts/crawl-news.ts --slug=xxx   # crawl 1 bài
 */
...
async function getNewsDriveFolder(): Promise<string> {
  ...
  const folderName = "Bang tin Hoi - Attachments"
...
function slugFromUrl(url: string): string {
  const match = url.match(/\/bang-tin-hoi\/([^/.]+)/)
  if (!match) {
...
function resolveImageSrc(src: string, sourceUrl: string): string[] {
  ...
  if (/\/bang-tin-hoi\/images\//i.test(natural)) {
    const root = natural.replace(/\/bang-tin-hoi\/images\//i, "/images/")
...
async function processImages(
  ...
  const folder = `tin-tuc/${monthFolder(publishedAt)}`
...
async function crawlAndInsert(article: JsonArticle, adminId: string): Promise<"ok" | "skip" | "error"> {
  ...
  const coverImageUrl = await uploadThumbnail(article.thumbnail, publishedAt)

  // 2. Fetch article page
  const encodedUrl = encodeURI(decodeURI(article.link))
  ...
  if (!mainContent) {
    ...
    await prisma.news.create({
      data: {
        title: article.title,
        slug,
        content: `<p><em>Nội dung đang được cập nhật. Xem bản gốc tại: <a href="${article.link}" target="_blank" rel="noopener noreferrer">${article.link}</a></em></p>`,
        excerpt: null,
        coverImageUrl,
        category: "GENERAL",
        isPublished: true,
  ...
  const imageStats = await processImages(mainContent, article.link, publishedAt)

  // 5. Process file attachments → Google Drive
  const fileStats = await processFileAttachments(mainContent, article.link)
...
async function main() {
  ...
  console.log("CRAWL TIN TỨC HỘI — hoitramhuongvietnam.org")
  console.log(`Mode: ${FORCE ? "FORCE re-crawl" : "skip đã tồn tại"}${SLUG_ARG ? ` | slug=${SLUG_ARG}` : ""}`)
  console.log("═".repeat(60))

  // Load articles JSON
  const jsonPath = resolve(__dirname, "bang-tin-hoi-articles.json")
  const articles: JsonArticle[] = JSON.parse(readFileSync(jsonPath, "utf-8").replace(/^\uFEFF/, ""))
  console.log(`📋 ${articles.length} bài tin tức từ JSON\n`)
  ...
  const toProcess = SLUG_ARG
    ? articles.filter((a) => slugFromUrl(a.link) === SLUG_ARG)
    : articles
